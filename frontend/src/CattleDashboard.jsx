...
const CattleDashboard = () => {
  const [cattle, setCattle] = useState(
    JSON.parse(localStorage.getItem("cattle")) || [
      { id: 1, name: "Bella", breed: "Holstein", age: 5, milk: 20, weight: 500 },
    ]
  );
  const [newCow, setNewCow] = useState({ name: "", breed: "", age: "", milk: 0, weight: 0 });
  ...
  return (
    <motion.div className="min-h-screen bg-white p-6 relative" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      {showEditModal && <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-10"></div>}
      <h1 className="text-5xl font-bold text-[#662929] text-center mb-6">🐄 Cattle Management</h1>
      <WeatherWidget />

      <motion.div className="bg-white shadow-2xl rounded-2xl p-6 max-w-6xl mx-auto" whileHover={{ scale: 1.02 }}>
        <table className="w-full border-collapse shadow-lg rounded-xl overflow-hidden">
          ...
          <tbody>
            {cattle.map((cow, index) => (
              <tr key={cow.id} className={`${index % 2 === 0 ? "bg-white" : "bg-white"} text-center hover:bg-[rgb(168,85,85,0.1)] transition`}>
                ...
                <td className="p-4 flex justify-center gap-3">
                  <button onClick={() => handleEditCow(cow)} className="text-yellow-500 hover:text-yellow-600">
                    <Edit size={22} />
                  </button>
                  <button onClick={() => handleDeleteCow(cow.id)} className="text-red-600 hover:text-red-700">
                    <Trash2 size={22} />
                  </button>
                </td>
              </tr>
      ...
      <motion.div className="bg-white shadow-xl rounded-2xl p-6 max-w-6xl mx-auto mt-6" whileHover={{ scale: 1.02 }}>
        <h2 className="text-2xl font-semibold mb-4 text-[#662929]">Add Cattle</h2>
        <div className="grid grid-cols-3 gap-4">
          {Object.keys(newCow).map((key) => (
            <div key={key}>
              <label className="block text-md font-medium text-gray-800">{key.charAt(0).toUpperCase() + key.slice(1)}</label>
              <input type="text" className="border p-3 w-full rounded-lg shadow-sm focus:ring-2 focus:ring-[#662929]" value={newCow[key]} onChange={(e) => setNewCow({ ...newCow, [key]: e.target.value })} />
            </div>
          ))}
        </div>
        <button className="bg-[#662929] text-white px-6 py-3 rounded-lg w-full mt-4 hover:bg-[rgb(102,41,41,0.8)] transition" onClick={handleAddCow}>Add Cattle</button>
      </motion.div>

      {showEditModal && (
        <div className="fixed inset-0 flex justify-center items-center z-20">
          <motion.div className="bg-white p-6 rounded-xl shadow-xl w-1/3 relative" initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.3 }}>
            <button className="absolute top-2 right-2 text-red-600 hover:text-red-700" onClick={() => setShowEditModal(false)}>
              <X size={26} />
            </button>
            <h2 className="text-2xl font-semibold mb-4 text-[#662929]">Edit Cattle</h2>
            <div className="grid grid-cols-2 gap-4">
              {Object.keys(editData || {}).map((key) => (
                <div key={key}>
                  <label className="block text-md font-medium text-gray-800">{key.charAt(0).toUpperCase() + key.slice(1)}</label>
                  <input type="text" className="border p-3 w-full rounded-lg shadow-sm focus:ring-2 focus:ring-[#662929]" value={editData[key]} onChange={(e) => setEditData({ ...editData, [key]: e.target.value })} />
                </div>
              ))}
            </div>
            <button className="bg-[#662929] text-white px-6 py-3 rounded-lg w-full mt-4 hover:bg-[rgb(102,41,41,0.8)] transition" onClick={handleUpdateCow}>Update</button>
          </motion.div>
        </div>
      )}
    </motion.div>
